...
- **История** за неделю или месяц: что было сделано и от чего отказались.
...
При создании тап по имени выбирает одного и ведёт дальше, как раньше; кнопка «Нескольким…»
открывает список с отметками. Состав можно поменять и потом — в карточке дела, кнопка
«Кто делает»: нажатие добавляет человека или снимает.
...
и дело закрывается, лишь когда отказались все. В расписании у такого дела поверху лента из
цветов исполнителей.

Повторяющимся дело на нескольких сделать нельзя — у повторяющегося исполнитель один.
