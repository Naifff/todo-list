...
public class TaskListPresenter {
    ...
    /**
     * ⚠️ Список — хронология без исключений, а протухшее из него уходит.
     *
     * <p>Что «уже прошло», говорит пометка в строке (⌛ у события, ❗️ у просроченного срока), а не
     * место в списке: раскладка «прошедшее в конец» с тридцатью делами читалась как сбитая
     * сортировка.
     */
    private TaskQuery query(Member viewer, TaskListView.Kind kind) {
        ...
        return query.withoutStale(startOfToday(viewer).minus(GRACE));
    }

    /**
     * Сколько прошедшее дело ещё висит в списке.
     *
     * <p>Неделя. Сперва было три дня и только для событий — сроки не протухали вовсе. 5 сентября
     * на проде это дало семнадцать строк середины августа, которые никто уже не закроет. Неделя —
     * потому что за неё дело либо переносят, либо оно перестаёт быть нужным; дальше оно только
     * копится.
     *
